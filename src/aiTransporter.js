...
const STATE_INITIALISING = function (creep) {
    log.creep(creep, 'Transporter starting up!', true);
    creep.memory.sourceIds = roomUtils.findEnergySourceIdsInRoom(creep.room);
    if (creep.memory.sourceIds === undefined) {
        log.creep(creep, 'ALERT! No container ids defined', true);
        creep.say('Need ids!');
    } else {
        creep.memory.state = 'GATHERING';
        log.creep(creep, 'Gathering from locations [' + JSON.stringify(creep.memory.sourceIds) + ']', true);
...
const STATE_TRANSPORTING = function (creep) {
    if (creep.carry.energy === 0) {
        ...
        if (total) {
            ...
            log.creep(creep, 'Heading to gather minerals!');
            return transitionToState(creep, 'FETCHING');
        } else {
            log.creep(creep, 'No minerals, time to get energy!');
